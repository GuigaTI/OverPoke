...
class Overlay(QWidget):
    ...
    def on_key_press(self, key):
        ...
        if parsed_hotkey.issubset(self.pressed_keys):
            self.increment()
...
class Manager(QWidget):
    ...
    def create_overlay(self):
        ...
        if not ok or not hotkey: return
        gif_path, _ = QFileDialog.getOpenFileName(self, "Selecionar GIF", "", "GIF Files (*.gif)")
    ...
    def reset_selected(self):
        index = self.list_widget.currentRow()
        if index >= 0: self.overlays[index].reset_counter()
    ...
    def change_theme(self):
        theme, ok = QInputDialog.getItem(self, "Selecionar Tema", "Escolha o tema:", list(THEMES.keys()), 0, False)
        if ok:
